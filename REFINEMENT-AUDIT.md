...
**🔄 Phase 13 in progress (started 2026-04-27): UI redesign via static-mockup loop.** A parallel static HTML sandbox at `static-mockup/` (six pages mirroring the live surface 1:1) is the design iteration target — not `netpulse-ui/templates/`. Reference design extracted from `anthropic.com` via Playwright MCP and substituted to original NetPulse content per the 14-row table in `docs/DESIGN-SPEC-ANTHROPIC-INSPIRED.md` §6. CSS uses heliodoron 3-layer font system (raw stacks → semantic aliases → 11 `.np-type-*` role classes) — typeface swap is a 3-line change, no component edits. **Files intentionally untracked in git** until the user signals "clear the iteration." Phase 13 has no audit-style line-items because it's a free-form design loop, not a defined deliverable list — the exit point is "design approved, port to Flask = Phase 14." Detail in [`REFINEMENT-PHASES.md` §Phase 13](./REFINEMENT-PHASES.md).
