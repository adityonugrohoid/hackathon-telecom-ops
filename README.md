...
3. **Watch the timeline animate** — `Classifier → Network Investigator → CDR Analyzer → Response Formatter`. Each entry shows its data source pills, the live tool calls (`query_events_jakarta(...)`, `query_cdr(...)`, `save_incident_ticket(...)`), and a tiny `🌐 via global` chip in the header showing which Vertex AI region answered. On a quota miss or 5s silent hang, the chip grows into `🌐 via global ⤳ us-central1` — failure visible as one extra hop, not a hard 500.
